...
public class OrderDAO {
  ...
  public static Integer initOrder(OrderDTO orderDTO) {
    ...
    try {
      String sql =
        "INSERT INTO person_order (person_id, address, phone, status, created_date) VALUES (?, ?, ?, ?, ?);";
  ...
  public static Order getOrderByIdAndUserId(int orderId, int userId) {
    Order order = null;
    try {
      Connection conn = ConnectionPool.getConnection();
  ...
  public static ArrayList<OrderDTO> getOrders() {
    ...
    try {
      ...
      String SQL_GET_ORDERS =
        "SELECT person_order.id, person_id, name, address, phone, status, created_date, total FROM person_order, person WHERE person.id = person_order.person_id";
      PreparedStatement ps = conn.prepareStatement(SQL_GET_ORDERS);
      ResultSet rs = ps.executeQuery();
